...
<body style="margin: 20px; font-family: Arial, sans-serif; background-color: #fcfcfc;">
   ...
   <jsp:include page="../admin-menu.jsp" />
    ...
    <!-- TABS -->
    <div class="tabs">
        <a class="tab active" onclick="openTab(event, 'Edition')">USER EDITION</a>
        <a class="tab" onclick="openTab(event, 'List')">USER LIST</a>
    </div>
        ...
        <form action="${pageContext.request.contextPath}/admin/user/update" method="post">
            <div style="display: flex; gap: 20px;">
                <div class="form-group">
                    ...
                    <input type="text" name="username" value="${user.id}" class="form-control" readonly style="background:#eee;" placeholder="Chọn User từ danh sách để sửa">
                </div>
                <div class="form-group">
            ...
            <div style="text-align: right; border-top: 1px solid #ff9800; padding-top: 15px;">
                <button type="submit" class="btn btn-update" ${!isEdit ? 'disabled style="opacity:0.5;"' : ''}>Update</button>
                <a href="${pageContext.request.contextPath}/admin/user/delete?id=${user.id}" class="btn btn-delete" onclick="return confirm('Xác nhận xóa người dùng này?');" style="${!isEdit ? 'pointer-events:none; opacity:0.5;' : ''}">Delete</a>
                <a href="${pageContext.request.contextPath}/admin/user/reset" class="btn btn-reset">Reset</a>
            </div>
        </form>
    </div>
